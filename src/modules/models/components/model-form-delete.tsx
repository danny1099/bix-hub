...
export const ModelFormDelete = (model: Model) => {
  ...
  const { mutateAsync, isPending } = trpc.model.delete.useMutation({
    onSuccess: () => utils.model.getAll.invalidate(),
  });

  const onSubmit = async () => {
    const { status, message } = await mutateAsync({ param: model.id });
