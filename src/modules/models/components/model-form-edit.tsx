...
import { modelWithIdSchema, type ModelWithIdSchema } from "@/modules/models/schema";
import { trpc, useUtils } from "@/trpc/client";

export const ModelFormEdit = (model: Model) => {
  ...
  const { mutateAsync } = trpc.model.edit.useMutation({
    onSuccess: () => utils.model.getAll.invalidate(),
  });

  const form = useForm<ModelWithIdSchema>({
    resolver: zodResolver(modelWithIdSchema),
    defaultValues: {
      id: model.id,
      name: model.name,
      url: model.url,
  ...
  const onSubmit = async (values: ModelWithIdSchema) => {
