...
import { Icon, Button, InputGroup, InputGroupAddon, InputGroupButton, InputGroupInput } from "@/shared/components";
...
export function ComboboxItem({ className, children, ...props }: ComboboxPrimitive.Item.Props) {
  return (
    <ComboboxPrimitive.Item
      ...
      className={cn(
        "data-highlighted:bg-accent data-highlighted:text-tertiary not-data-[variant=destructive]:data-highlighted:**:text-accent-foreground text-2xs relative flex w-full cursor-default items-center gap-2 rounded-md py-1 pr-8 pl-1.5 outline-hidden select-none data-disabled:pointer-events-none data-disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
        className
      )}
      {...props}
    >
    ...
    >
      ...
      {showRemove && (
        <ComboboxPrimitive.ChipRemove
          ...
          render={<Button variant="ghost" size="icon" icon="close" className="size-4" />}
        />
      )}
    </ComboboxPrimitive.Chip>
  );
